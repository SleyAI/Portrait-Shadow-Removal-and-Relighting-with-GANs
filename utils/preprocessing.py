...
def vis_parsing_maps_binary(im, parsing_anno, stride, save_im=True, save_path='vis_results/parsing_map_on_im.jpg'):
    # Colors for all 20 parts
    part_colors = [[0, 0, 0],
    ...
    vis_parsing_anno = cv2.resize(vis_parsing_anno, None, fx=stride, fy=stride,
                                  interpolation=cv2.INTER_NEAREST)  # [512,512]
    vis_parsing_anno_color = np.zeros((vis_parsing_anno.shape[0], vis_parsing_anno.shape[1], 3)) + 255
...
def vis_parsing_maps_five_organs(im, parsing_anno, stride, save_im=False,
...
def vis_parsing_maps_binary_hair(im, parsing_anno, stride, save_im=False,
                                 save_path='vis_results/parsing_map_on_im.jpg'):
    # Colors for all 20 parts   # 24
    part_colors = [[0, 0, 0],
                   [255, 255, 255], [255, 255, 255], [255, 255, 255], [255, 255, 255], [255, 255, 255],
...
def evaluate(respth='./res/test_res2', dspth='./data', cp='model_final_diss.pth', seg_type='multiple',
             save_binary_mask=True, stride=1):
        ...
        if os.path.isdir(dspth):
            for image_path in os.listdir(dspth):
                ...
                if seg_type == 'multiple':
                    ...
                    mask = vis_parsing_maps_binary(image, parsing, stride=1, save_im=save_binary_mask,
                                                   save_path=osp.join(respth, image_path))
                elif seg_type == 'five_organs':
                    mask = vis_parsing_maps_five_organs(image, parsing, stride=1, save_im=save_binary_mask,
                                                        save_path=osp.join(respth, image_path))
                elif seg_type == 'binary+hair':
                    mask = vis_parsing_maps_binary_hair(image, parsing, stride=1, save_im=save_binary_mask,
                                                        save_path=osp.join(respth, image_path))
